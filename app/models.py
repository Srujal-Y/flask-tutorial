...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def avatar(self, size):
        identifier = (self.email or self.username or '').lower()
        digest = md5(identifier.encode('utf-8')).hexdigest()
    ...
    def launch_task(self, name, description, *args, **kwargs):
        try:
            rq_job = current_app.task_queue.enqueue(f'app.tasks.{name}',
                                                    self.id, *args, **kwargs)
        except redis.exceptions.RedisError:
            current_app.logger.exception('Could not enqueue task %s', name)
            return None
